...
const AboutSection = () => {
  ...
  useEffect(() => {
    try {
      ...
      setAbout(data);
      setIsLoading(false); // Add this line to stop loading once data is set
    } catch (error) {
      console.log(error);
      setIsLoading(false);
    }
          ...
          {isLoading ? (
            ...
            <>
              {about.map((item: AboutProps, index: number) => (
                <p
                  key={index}
                  className="flex flex-col w-full text-base text-justify lg:text-start lg:leading-8 text-neutral-200"
                >
        ...
        <div className="w-full mt-8">
          <AnimationContainer customClassName="w-full flex flex-col">
            ...
            <div className="w-full mb-8 space-y-5">
              ...
              >
                Looking ahead, I&apos;m excited about creating 3D websites. I
                want to make online spaces that feel more real and fun for
                users.
              </motion.p>
              <motion.p
                initial={{ opacity: 0, y: 20 }}
                ...
                className="text-base leading-8 text-neutral-200"
              >
                I&apos;m also curious about machine learning, which is like
                teaching computers to learn and do smart things. I dream of
                building smart systems that work hand-in-hand with people,
                making tech a helpful companion in our daily lives.
              </motion.p>
              <motion.p
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
